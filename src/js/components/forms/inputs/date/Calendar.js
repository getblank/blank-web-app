...
class Calendar extends React.Component {
    constructor(props) {
        ...
        this.state = Object.assign(
            { year: this.moment().year(), month: this.moment().month() },
    ...
    getWeekDays() {
        return [1, 2, 3, 4, 5, 6, 7].map(d => (
            <div className={"week-day" + (d === 7 ? " last" : "")} key={"dw-" + d}>
                {this.moment()
                    .isoWeekday(d)
                    .format("dd")}
            </div>
    ...
    render() {
        ...
        for (let i = 0; i < 42; i++) {
            ...
            var cn = classNames("calendar-day", {
                ...
                "range-start": Array.isArray(this.state.selected) && start.isSame(this.state.selected[0], "day"),
                "in-range":
            ...
            if (i % 7 === 6) {
                controls.push(<div key={"w-" + Math.floor(i / 7)}>{week}</div>);
            }
            start.add(1, "days");
        }
        return (
